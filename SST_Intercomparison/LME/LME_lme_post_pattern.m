function [out,out_rnd] = LME_lme_post_pattern(M,lme,D,P)
    ...
    if is_pattern == 1
        ...
        out.bias_fixed_pattern = out.bias_fixed((N_grp+1):(2*N_grp));
        out.bias_fixed = out.bias_fixed(1:N_grp);

        out.bias_fixed_pattern_std = out.bias_fixed_std((N_grp+1):(2*N_grp));
        out.bias_fixed_std = out.bias_fixed_std(1:N_grp);

        if P.do_sampling ~= 0
            out.bias_fixed_pattern_random = out.bias_fixed_random((N_grp+1):(2*N_grp),:);
            out.bias_fixed_random = out.bias_fixed_random(1:N_grp,:);
        end
    end
